...
| Do tests cover key success, rejection, failure, and recovery paths? | Success, rejection, named runtime failures, Reaction dispatch failures, projection deserialization failures, source-aware duplicate/out-of-order delivery, partial projection failure, checkpoint failure, and rebuild reset are covered. Transport consumption and acknowledgement remain adapter work. |
...
| What debt or ambiguity should be resolved before Phase 7? | Projection source identity and checkpoint policy are now explicit. EventStore adapter contracts, NATS key/subject rules, ViewStore index persistence semantics, transport consumption, and journal retry/idempotency decisions remain. |
...
| Source-aware projection application | `source_aware_*` projection runtime tests | Application identity includes projection type, source Resource stream, and target View key; duplicates and older same-source positions cannot rewrite a View. |
| Event checkpoint completion | `event_checkpoint_waits_for_every_required_projection_and_partial_retry_is_safe` | One Event checkpoint advances only after all required handlers succeed, while successful View writes remain safe on retry. |
| Checkpoint crash window | `checkpoint_save_failure_retries_without_regressing_the_written_view` | A checkpoint failure after a View write retries the checkpoint without reapplying that View write. |
| Projection rebuild | `rebuild_pauses_delivery_and_idempotently_resets_views_metadata_and_checkpoint` | Normal delivery pauses while selected Views, application metadata, and checkpoint state reset before replay. |
...
| Projection source deserialization fails | Runtime returns `ProjectionExecutionError::SourceEventDeserialization`; dispatcher wraps named failure details. | No View write for that handler and no Event checkpoint advance. | View may lag behind Resource Events until replay succeeds. | Projection failure and checkpoint contract tests. | Transport adapter retries from its last durable checkpoint without acknowledging the failed Event. |
| View write fails | Runtime returns `ProjectionExecutionError::ViewStore`; successful sibling View applications retain source metadata. | Resource Events remain unchanged; the failed target is not marked applied; Event checkpoint does not advance. | Delivery must retry until every required View succeeds. | Partial multi-handler retry and named ViewStore error tests. | NATS KV adapters preserve per-View source application metadata; transport acknowledgement remains adapter-owned. |
| Event checkpoint write fails after all Views succeed | Dispatcher returns `ProjectionDispatchError::Checkpoint`. | Successful View documents and application metadata remain; checkpoint remains at its prior position. | Delivery repeats after the crash window. | Checkpoint save failure contract test. | Retry skips already-applied View targets and saves the Event checkpoint before adapter acknowledgement. |
| Rebuild is active | Dispatcher returns `ProjectionDispatchError::RebuildInProgress` for normal delivery. | Selected Views, application metadata, and checkpoint are reset before replay. | Concurrent normal delivery could otherwise repopulate stale state. | Paused idempotent rebuild contract test. | Adapter must stop normal delivery while the framework rebuild handle is active. |
...
| RV3 | Resolved | Projection source and checkpoint semantics are explicit. | `ProjectionContext`, source-aware per-View application metadata, `ProjectionCheckpointStore`, checkpoint dispatch errors, and paused rebuild selection are covered by contract tests. | A subscription adapter can resume from one Event-level checkpoint without reapplying successful View writes. | Keep cursor interpretation and acknowledgement in the future transport adapter; do not add an implicit core acknowledgement. |
...
| RV6 | Medium | Projection dispatch failures are returned to caller but not journaled. | Event checkpoints record only completed source positions; failed handler/checkpoint details remain caller-visible errors. | Operators need adapter/tooling to observe failures. | Keep failures out of Resource Events; transport observability may wrap the named errors without creating a ProjectionJournal implicitly. |
...
6. Preserve the explicit Event-level checkpoint and source-aware View application contract; transport cursors and acknowledgements remain adapter-owned.
