...
Every role report records its OpenCode role task ID, role session ID, role, issue, branch, base and head revisions, exact changed paths, exact commands, command results, and blocker status. This provenance is passed unchanged to later roles and copied into the pull request body or append-only comments without rewriting accepted evidence.

Red evidence includes the focused command, output, intended failure reason, exact changed test or fixture paths, red commit SHA, and PR URL. Green evidence includes the focused pass, `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test --all`, exact implementation and documentation paths, documentation or no-docs note, architecture impact, limitations, red commit SHA, green commit SHA, and PR URL. Readiness evidence includes the review task ID and Reviewer session ID, reviewed pull request range, findings ordered by severity, exact independently checked commands and command results, CI state, residual risks, blocker status, both commit SHAs, and PR URL.

The Publisher appends complete red evidence, green evidence, and readiness evidence as new append-only comments on both the GitHub issue and pull request without rewriting any prior comment. Each cumulative evidence comment carries all currently available role task IDs, role session IDs, exact changed paths, red commit SHA, green commit SHA, exact commands, command results, review task ID, blocker status, and PR URL; fields belonging to later phases are marked pending until the next comment.

Issue #147 / draft PR #148 is a bootstrap exception: the initial pull request existed before the Publisher role was introduced, so append-only evidence comments reconstruct this run. All future runs must follow the full sequence red -> Publisher -> green -> Publisher -> Reviewer PR review -> Publisher readiness publication.
...
The Reviewer is read-only: its OpenCode agent denies edits and permits shell access only for the exact current-branch inspection, current PR inspection, and quality commands. `elbmesh-reviewer` is the single active final PR role that reports merge readiness after findings and evidence checks; it cannot modify files, update GitHub state, or merge. Findings return to a fresh Implementer session and pass through another green publication and review cycle.
...
Human merge authority is absolute. The Reviewer may report merge readiness after review has no blocking findings and required CI passes; the Publisher may then mark the pull request ready and report its URL, while the Orchestrator only coordinates state. Only the human may perform the final review and merge. The Orchestrator requests `status:merged` after observing the human merge, and the human applies it.
