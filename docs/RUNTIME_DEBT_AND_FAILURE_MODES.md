...
| D8 | Medium | `ActionFailed.failure_details` is structurally present but semantically thin. | `ActionJournalRecord::ActionFailed`, `action_failed_record` | Classification is useful, but diagnostics are weak for operators and future retry policy. | Trace tests that assert stable failure details or explicit trace gaps. | Phase 2.5 trace model, Phase 7/8 adapter overlays |
...
| Handler runtime failure | Executor journals `ActionFailed(HandlerRuntime)` best-effort and returns original typed runtime error. | `ActionCalled`, `ActionFailed` | Failure details are sparse. | Executor tests cover wrong-resource runtime failure. | Trace model should make failure context inspectable. | Phase 2.5 trace model |
...
| EventStore load failure | Executor journals `ActionFailed(EventStore)` best-effort and returns EventStore error. | `ActionCalled`, `ActionFailed` | Adapter errors need richer classification/details. | Executor test covers load failure. | NATS adapter must expose meaningful error details. | Phase 7 adapters |
| Resource replay/deserialization failure | Executor journals `ActionFailed(Resource)` best-effort and returns Resource error. | `ActionCalled`, `ActionFailed` | Replay validation and malformed event handling need stronger contracts. | Executor test covers malformed historical event. | Adapter contract must reject wrong-stream/schema/gap cases. | Phase 3/7 contracts |
| EventStore append failure | Pending Resource Events are not persisted; executor journals `ActionFailed(EventStore)` best-effort. | `ActionCalled`, `ActionFailed`; attempted pending events are not generally durable | Operators may not see which pending events failed unless details/overlay capture them. | Executor test captures fake append batch. | Trace or adapter overlay should show attempted append safely. | Phase 2.5 trace model, Phase 7 overlays |
...
Should ActionFailed.details remain empty, carry stable diagnostics, or move diagnostics to overlays?
