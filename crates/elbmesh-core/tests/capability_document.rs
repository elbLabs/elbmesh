...
fn capability_document_projects_manifest_identity_and_generator_metadata() {
    ...
    assert_eq!(capabilities.manifest_hash, "fnv1a64:6a2d2e43edfa832f");
}
...
#[test]
fn committed_generated_capability_outputs_match_manifest_fixture() {
    let capabilities = CapabilityDocument::from_manifest(&offer_manifest());
    let generated_json = format!(
        "{}\n",
        serde_json::to_string_pretty(&capabilities).expect("serialize generated capabilities")
    );

    assert_eq!(
        include_str!("../../../docs/resource-capabilities.json"),
        generated_json
    );
    assert_eq!(
        include_str!("../../../docs/RESOURCE_CAPABILITIES.md"),
        capabilities.to_markdown()
    );
    assert_eq!(
        include_str!("../../../docs/resource-bindings.rs"),
        capabilities.to_rust_binding_stubs()
    );
}
